...
const marcaFord = cars.filter(car => car.brand === "Ford")


/*const find = (person) => car.id === person.car*/

const personasFord = marcaFord.map(ford => {
    const propietario = persons.find (person => person.car === ford.id )
...
/*- 7. ¡Bonus point! Obtén un array con todas las personas en el
 que cada persona tenga toda
*       la info de su coche. Ejemplo a continuación:*/


const personasConcoche = persons.map(person => {

const personCar = {...person,pet : {...person.pet}}

    /*busco el coche cuyo coche coincida con el coche de la persona de esta vuelta del map*/
const coche = cars.find(coche => coche.id === person.car)
...
console.log("7-personas con info del coche:",personasConcoche)
...
const cochesDeEspanoles = espanoles.map((espanol) => {
